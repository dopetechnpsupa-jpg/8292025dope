...
const SplashScreen = ({ isVisible, onComplete }: { isVisible: boolean, onComplete: () => void }) => {
  ...
  return (
             <div className="w-full h-screen bg-[#feda00] flex items-center justify-center">
    ...
    <>
      ...
      {!showSplash && isAppReady && (isLoading || isClientLoading || isDataLoading || isNavigating) && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-40">
          <div className="text-center">
                         <div className="w-12 h-12 border-4 border-[#feda00] border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
                          <p className="text-[#feda00] font-semibold text-lg mb-2">Loading...</p>
         ...
         <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2">
          <nav className="flex items-center justify-between h-auto min-h-16">
            {/* Left Side - Mobile Menu Toggle (mobile only) */}
               ...
               <ClientOnly fallback={<button className="p-2 rounded-lg hover:bg-gray-100 transition-colors" aria-label="Menu"><Menu className="w-5 h-5 text-gray-700" /></button>}>
                 <button
            ...
            <div className="flex items-center space-x-3 min-w-0 flex-1">
               ...
               {/* Brand */}
               <div className="ml-3">
                 <h1 className="text-lg font-semibold text-gray-900">DopeTech</h1>
                 <p className="text-xs text-gray-500">Premium Tech Gear</p>
               </div>
            </div>
             ...
             <div className="hidden md:flex items-center justify-center absolute left-1/2 transform -translate-x-1/2">
               <div className="relative w-[28rem]">
                 ...
                 <input
                   ...
                   className="w-full pl-10 pr-4 py-2 text-sm bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#feda00] text-gray-900 placeholder-gray-500"
                 />
               </div>
             </div>
             ...
             <div className="flex items-center justify-end space-x-4 flex-shrink-0">
               ...
               >
                 ...
                 <ClientOnly fallback={null}>
                   {getCartCount() > 0 && (
                     <span className="absolute -top-2 -right-2 bg-[#feda00] text-black text-xs rounded-full w-5 h-5 flex items-center justify-center font-medium">
                       {getCartCount()}
                     </span>
                   )}
                 </ClientOnly>
               ...
               >
                 ...
                 <div className="relative p-4">
                   ...
                   <input
                     ...
                     value={searchDraft}
                     onChange={(e) => setSearchDraft(e.target.value)}
                     className="w-full pl-12 pr-4 py-3 text-base bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#feda00] text-gray-900 placeholder-gray-500"
                     autoFocus
                   />
                 </div>
               </div>
                     ...
                     {/* Mobile Menu */}
           {isMobileMenuOpen && (
             <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-200 rounded-xl shadow-lg lg:hidden z-50" data-mobile-menu>

               {/* Mobile Brand */}
               <div className="text-center mb-3 pb-3 border-b border-gray-200 p-4">
                 <h1 className="text-lg font-semibold text-gray-900">DopeTech</h1>
                 <p className="text-xs text-gray-500">Premium Tech Gear</p>
               </div>
               ...
               <div className="space-y-2 p-4">
                 {categories.map((category) => (
                   <button
                     ...
                     className={`w-full flex items-center space-x-2 px-3 py-2.5 rounded-full transition-all duration-200 ${
                       selectedCategory === category.id
                         ? "bg-[#feda00] text-black shadow-md"
         ...
         <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-8">
            ...
            {/* Tagline removed - now in navigation */}
             ...
             <div className="w-full mx-auto mt-2 mb-2 sm:mb-8">
               <ClientOnly fallback={
                 <div className="flex items-center justify-center h-64 bg-gray-200 rounded-2xl">
                   <div className="text-center">
                     <div className="w-12 h-12 border-4 border-[#feda00] border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
                     <p className="text-gray-600 font-semibold">Loading Carousel...</p>
                   </div>
                 </div>
               }>
                 {heroLoading ? (
             ...
             <div className="w-full mx-auto mt-1 sm:mt-6 mb-2 sm:mb-6">
                               <div className="text-center mb-3 sm:mb-6 px-2 sm:px-4">
                                   <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">
                     <span className="text-[#feda00]">Dope</span> Picks
                   </h2>
             ...
             <div className="mt-2 sm:mt-4 mb-2 sm:mb-6 mx-0 sm:mx-4 md:mx-6 lg:mx-8">
              <DopeDailyShowcase 
                products={products}
                onAddToCart={handleAddToCartWithTracking}
                className="w-full"
              />
                         ...
                         {/* Dope Categories Header - Mobile Optimized Spacing */}
                           <div className="text-center mb-2 sm:mb-4 px-2 sm:px-4">
                                                                                                                             <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">
                     <span className="text-[#feda00]">Dope</span> Categories
                   </h2>
                <p className="text-lg text-gray-600">
              ...
              <div ref={categorySectionRef} className="mb-3 sm:mb-12">
                ...
                <div className="flex overflow-x-auto scrollbar-hide gap-2 sm:gap-3 sm:flex-wrap sm:justify-center w-full px-4">
                  {categories.map((category, index) => (
                    <div key={category.id} className="relative flex-shrink-0 sm:flex-shrink">
                      <button
                        onClick={() => handleCategoryClick(category.id)}
                                                className={`flex items-center space-x-2 px-4 py-2 rounded-full transition-all duration-200 cursor-pointer text-sm font-medium whitespace-nowrap ${
                           selectedCategory === category.id
                             ? "bg-[#feda00] text-black shadow-md"
             ...
             {(isLoading || isDataLoading) ? (
               // Loading skeletons
                 ...
                 <div key={product.id} data-product-id={product.id} className="group bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200 cursor-pointer h-full flex flex-col">
                   <div 
                     className="relative overflow-hidden rounded-t-lg bg-gray-50 flex-1 flex flex-col"
                   ...
                   >
                     ...
                     <div className="relative aspect-square overflow-hidden bg-gray-50 flex-shrink-0">
                      ...
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-all duration-300 flex items-center justify-center opacity-0 group-hover:opacity-100">
                        <button
                          ...
                          className="bg-[#feda00] text-black hover:bg-[#feda00]/90 transition-all duration-200 font-medium px-4 py-2 rounded-lg shadow-lg"
                    ...
                    <div className="p-4 flex-1 flex flex-col justify-between">
                      {/* Product Name */}
                      <h3 className="text-gray-900 text-sm font-medium mb-2 line-clamp-2">
                        {product.name}
                      </h3>
       ...
       {Object.keys(dopeArrivals).length > 0 && (
         <section className="py-16 bg-white">
           <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
               ...
               {/* Section Header */}
               <div className="text-center mb-8 px-0 sm:px-4">
                 <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">
                   Recent <span className="text-[#feda00]">Drops</span>
                 </h2>
                 <p className="text-lg text-gray-600">
                   Latest arrivals from our collection
                 </p>
               </div>

                             {/* Categories Grid */}
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 px-0 sm:px-6 md:px-8">
                 {Object.entries(dopeArrivals).map(([categoryName, categoryProducts], categoryIndex) => (
                   <div
                     key={categoryName}
                     className="group relative bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-all duration-300 overflow-hidden"
                   >
                     {/* Category Header */}
                     <div className="bg-gray-50 p-4 sm:p-6 border-b border-gray-200">
                       <div className="flex items-center justify-between">
                         ...
                         <div className="bg-[#feda00]/20 text-[#feda00] text-xs px-2 py-1 rounded-full font-semibold">
                           {categoryProducts.length}
                         </div>
                       </div>
                       <p className="text-sm text-gray-600 mt-1">Latest arrivals</p>
                     </div>
                     ...
                     <div className="p-4 sm:p-6">
                       <div className="grid grid-cols-2 gap-3">
                         {categoryProducts.slice(0, 4).map((product, productIndex) => (
                           <div
                             key={product.id}
                             className="group/item relative bg-white rounded-lg border border-gray-200 overflow-hidden hover:shadow-md transition-all duration-300 cursor-pointer"
                             onClick={() => handleProductNavigation(product.id.toString())}
                           >
                             {/* Product Image */}
                             ...
                             <div className="p-2 bg-white">
                               <h4 className="text-xs font-semibold text-gray-900 group-hover/item:text-[#feda00] transition-colors duration-300 line-clamp-2 leading-tight">
                                 {product.name}
                               </h4>
                       ...
                       {categoryProducts.length > 4 && (
                         <div className="mt-4 pt-4 border-t border-gray-200">
                           <button className="w-full text-center text-[#feda00] hover:text-[#feda00]/80 text-sm font-medium transition-colors duration-300 bg-[#feda00]/10 hover:bg-[#feda00]/20 py-2 rounded-lg">
        ...
        <section className="py-16 bg-gray-50">
         <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
           <div className="w-full mx-auto">
             ...
             <div className="text-center mb-8 px-0 sm:px-4">
               <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">
                 Weekly <span className="text-[#feda00]">Picks</span>
               </h2>
               <p className="text-lg text-gray-600">
             ...
             {/* White Container - Products Only */}
             <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sm:p-8 lg:p-10 mx-0 sm:mx-4 md:mx-8 lg:mx-12">
            
                         {/* Product Grid - Images and Text Combined */}
             <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 mx-auto px-4 max-w-7xl">
               {weeklyPicks.map((product, index) => (
                 <div key={`weekly-pick-${product.id}`} className="w-full max-w-xs mx-auto">
                   {/* Image Section */}
                   <div className="group relative mb-3">
                     <div 
                       className="relative bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 cursor-pointer border border-gray-200"
                       onClick={() => handleProductNavigation(product.id.toString())}
                     >
                       {/* Full Image Section */}
                       <div className="relative aspect-square overflow-hidden bg-gray-100 max-w-full">
                         <img
                           src={getPrimaryImageUrl(product)}
                           alt={product.name}
                         ...
                         <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-all duration-300 flex items-center justify-center opacity-0 group-hover:opacity-100">
                           <button
                             ...
                             className="bg-[#feda00] text-black hover:bg-[#feda00]/90 transition-all duration-200 font-semibold px-4 py-2 rounded-lg"
                             disabled={!product.in_stock}
                           >
                             Add to Cart
                           </button>
                   ...
                   <div className="text-center">
                     ...
                     <h3 className="text-gray-900 font-bold text-base sm:text-lg md:text-xl lg:text-2xl mb-2 line-clamp-2">
                       {product.name}
                     </h3>
                     
                     {/* Price Section */}
                     <div className="flex flex-col items-center gap-1">
                       <span className="text-lg sm:text-xl md:text-2xl text-gray-900 font-bold">
                         Rs {product.discount > 0 ? Math.round(product.original_price * (1 - product.discount / 100)).toLocaleString() : product.price.toLocaleString()}
                       </span>
                       {(product.original_price > product.price || product.discount > 0) && (
        ...
        <section className="py-16 bg-white">
         <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
           ...
           <div className="text-center mb-8 px-2 sm:px-4">
                          <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">
                Dope <span className="text-[#feda00]">Recommendations</span>
              </h2>
             <p className="text-lg text-gray-600">
           ...
           <div className="w-full mx-auto rounded-2xl overflow-hidden bg-white shadow-sm border border-gray-200 px-4">
               ...
               {dopePicks.length > 0 ? (
                 ...
                 >
                   ...
                   >
                     ...
                     <div className="absolute inset-0 bg-gradient-to-r from-gray-900/60 to-transparent rounded-2xl" />
                   </div>
                   ...
                   <div className="absolute inset-0 flex items-end">
                     <div className="p-4 sm:p-6 w-full">
                       ...
                       <p className="text-sm sm:text-base md:text-lg text-[#feda00] font-semibold drop-shadow-lg">
                   ...
                   <div className="absolute inset-0 bg-[#feda00]/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                     <div className="bg-white/90 backdrop-blur-sm rounded-xl px-4 py-2 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                       <p className="text-gray-900 text-sm font-medium">View Product</p>
                     </div>
                   </div>
                 </div>
               ) : (
                 ...
                 <div className="w-full h-full bg-gray-100 flex items-center justify-center relative overflow-hidden">
                   <div className="text-center p-6">
                     <div className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 lg:w-28 lg:h-28 mx-auto mb-4 bg-[#feda00] rounded-full flex items-center justify-center">
                   ...
                   <div className="absolute inset-0 opacity-10">
                     <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-r from-[#feda00]/20 via-transparent to-[#feda00]/20 animate-pulse"></div>
                   </div>
                 </div>
               )}
             </div>
      ...
      {cartOpen && (
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="relative ml-auto w-full max-w-[85vw] sm:max-w-sm md:max-w-md bg-white shadow-2xl rounded-l-3xl border-l border-[#feda00]/20">
            <div className="flex flex-col h-full">
              {/* Enhanced Cart Header - Mobile Optimized */}
                             <div className="flex items-center justify-between p-4 sm:p-6 border-b border-[#feda00]/20">
                <h2 className="text-lg sm:text-xl font-bold text-gray-900">Shopping Cart</h2>
                <button
                  onClick={() => setCartOpen(false)}
                  className="p-3 sm:p-3 hover:bg-[#feda00]/10 rounded-full transition-colors touch-target"
              ...
              {cart.length > 0 && (
                                 <div className="sticky top-0 z-10 border-b border-[#feda00]/20 p-4 sm:p-6 shadow-lg">
                  <div className="flex justify-between items-center mb-4 sm:mb-4">
                    <span className="text-base sm:text-lg font-semibold text-gray-900">Total:</span>
                                         <span className="text-lg sm:text-xl md:text-2xl price-proxima-nova text-[#feda00]">
                      Rs {getCartTotal().toFixed(2)}
                    </span>
                  </div>
                  <button 
                    onClick={handleCheckout}
                    className="w-full bg-[#feda00] hover:bg-[#feda00]/90 text-black py-4 sm:py-4 px-4 sm:px-6 rounded-xl font-bold hover:shadow-xl transition-all duration-300 transform hover:scale-[1.02] text-base sm:text-base touch-target"
              ...
              <div className="flex-1 overflow-y-auto p-3 sm:p-4 md:p-6 scrollbar-hide">
                {cart.length === 0 ? (
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    {cart.map((item) => (
                                             <div key={item.id} className="bg-white border border-[#feda00]/20 rounded-2xl p-3 sm:p-4 space-y-3 sm:space-y-4">
                        <div className="flex items-start space-x-3 sm:space-x-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <h3 className="text-kelpt-a2 text-sm sm:text-base line-clamp-2 leading-tight text-gray-900 mb-1">{item.name}</h3>
                                                         <p className="text-[#feda00] price-proxima-nova text-base sm:text-lg mb-2">Rs {item.price}</p>
                            
                            {/* Quantity Controls - Mobile Optimized */}
                            <div className="flex items-center space-x-2 sm:space-x-3">
                              <button
                                onClick={() => updateQuantity(item.id, item.quantity - 1)}
                                className="p-2 sm:p-2.5 hover:bg-[#feda00]/20 rounded-lg transition-colors touch-target"
                                style={{ minHeight: '40px', minWidth: '40px' }}
                              >
                                <Minus className="w-4 h-4 sm:w-5 sm:h-5 text-gray-700" />
                              </button>
                              <span className="w-8 sm:w-10 text-center font-bold text-gray-900 text-base sm:text-lg">{item.quantity}</span>
                              <button
                                onClick={() => updateQuantity(item.id, item.quantity + 1)}
                                className="p-2 sm:p-2.5 hover:bg-[#feda00]/20 rounded-lg transition-colors touch-target"
                                style={{ minHeight: '40px', minWidth: '40px' }}
                          ...
                          <div className="flex flex-col space-y-2 flex-shrink-0">
                            <button
                              ...
                              className="p-2 sm:p-2.5 hover:bg-[#feda00]/20 rounded-lg text-[#feda00] transition-colors touch-target"
                              style={{ minHeight: '40px', minWidth: '40px' }}
                            >
                          ...
                          <div className="pl-0 sm:pl-20 space-y-2">
                            {item.selectedColor && (
                              <div className="flex items-center space-x-2">
                                ...
                                <span className="text-xs sm:text-sm font-medium text-gray-900">{item.selectedColor}</span>
                              </div>
                            )}
                            {item.selectedFeatures && item.selectedFeatures.length > 0 && (
                              <div className="flex items-center space-x-2">
                                <span className="text-xs sm:text-sm text-gray-500">Features:</span>
                                <span className="text-xs sm:text-sm font-medium text-gray-900">{item.selectedFeatures.join(', ')}</span>
                              </div>
                            )}
                          </div>
                        )}
             ...
             {/* Footer - Enhanced Spacing */}
               <footer className="bg-[#feda00] py-6 sm:py-8 md:py-10 lg:py-12 border-t-2 border-[#feda00]">
         <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
           <div className="flex flex-col md:flex-row justify-between items-center">
             <div className="flex items-center space-x-2 sm:space-x-3 md:space-x-4 mb-6 sm:mb-8 md:mb-0">
               ...
               <span className="text-xs sm:text-sm md:text-base text-black font-medium">© 2025 DopeTech Nepal. All rights reserved.</span>
             </div>

             <div className="flex flex-wrap justify-center gap-4 sm:gap-6 md:gap-8 lg:gap-10">
               <a href="/terms" className="text-xs sm:text-sm md:text-base text-black hover:text-gray-800 transition-colors font-medium">
                 Terms & Conditions
               </a>
               <a href="/support" className="text-xs sm:text-sm md:text-base text-black hover:text-gray-800 transition-colors font-medium">
                 Support
               </a>
             </div>
           </div>
